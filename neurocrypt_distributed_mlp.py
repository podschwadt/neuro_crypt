...
def validation_split( x_train, y_train, split=0.0 ):
...
for name, n_filters, l1 in parameters:
    ...
    (x_train, y_train), _ = D.load( name, as_torch_dataset=False, test_size=0.0, seed=65143201 )
    ...
    def create_model( seed=65440 ):
        ...
        init = tf.keras.initializers.GlorotUniform( seed=65440 )
        model.add( tf.keras.layers.Dense( 50, activation='relu', input_shape=x_train.shape[ 1: ], kernel_initializer=init ) )
        ...
        # model.compile( optimizer=opt, loss=tf.keras.losses.BinaryCrossentropy(), metrics=[ 'accuracy', 'AUC' ] )
        ...
        while True:
            ...
            for e in range( glabal_iterations ):
                ...
                for id in models:
                    ...
                    metrics.append( model.evaluate( data_folds[ id ][0], data_folds[ id ][ 1 ], verbose=0 ) )
                metrics = np.array( metrics )
                print( 'epoch {}, loss {:.3f}, acc {:.3f}, auc {:.3f}'.format( e, *np.mean( metrics, axis=0 ) ) )
                if plot:
            ...
            for _, model in models.items():
                ...
                loss, accuracy, auc_ = model.evaluate( data_folds[ holdout_id ][ 0 ], data_folds[ holdout_id ][ 1 ] )
                f1.append( f1_score( data_folds[ holdout_id ][ 1 ], y_pred  > 0.5 )  )
                auc.append( auc_ )
                acc.append( accuracy )
            return f1, auc, acc # while loop ends
    ...
    for fold_id in range( n_sites ):
        f1, auc, acc = train_distributed( data_folds, site_workers, holdout_id=fold_id )
        print_fn( 'fold {}\t'.format( fold_id ), f1, auc, file= name + '.log' )
        f1s.append( f1 )
        aucs.append( auc )
        accs.append( acc )
    tf.keras.backend.clear_session()
    pickle.dump( { 'f1':f1s, 'auc': aucs, 'acc': accs },  open( name + '_neurcrypt.pik', 'wb' ) )
