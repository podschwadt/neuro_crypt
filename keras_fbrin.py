...
# load data
(x_train, y_train), test_data = load( data_name, as_torch_dataset=False, test_size=0.5 )
print( x_train.shape )
x_train = np.moveaxis( x_train, 1 , 2 )
test_data = ( np.moveaxis( test_data[ 0 ], 1 , 2 ), test_data[ 1 ] )
print( x_train.shape )
print( test_data[ 0 ].shape, test_data[ 1 ].shape )
data_folds = build_cross_validation_folds( x_train, y_train, folds=1 )
train_mean = []
train_std = []
test_mean = []
test_std = []
...
for filters in n_filters:
    ...
    for i, ( (x_train, y_train), ( x_test, y_test ) ) in enumerate( data_folds ):
        ...
        while True:
            ...
            if val_auc > auc_threshold - decay:
                break
            decay += 0.01
            if val_auc > best:
              model.save( 'model.h5' )
        ...
        holdout_auc.append( model.evaluate( test_data[ 0 ], test_data[ 1 ], verbose=0 )[ 1 ] )
    ...
    mean = np.mean( holdout_auc )
    std = np.std( holdout_auc )
    holdout_mean.append( mean )
    holdout_std.append( std )
    print_fn( 'holdout\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}'.format(  mean, std, np.min( test_auc ), np.max( test_auc ) ),  file=data_name +'.log' ) 
    print_fn( 'params\t{}'.format( tf.python.keras.utils.layer_utils.count_params( model.trainable_weights ) ),  file=data_name +'.log' ) 

plt.ioff()
plt.ylim( (0,1) )
plt.errorbar( x=[ 0 ], y=train_mean, yerr=train_std, label='train', fmt='go', ecolor='k' )
plt.errorbar( x=[ 1 ], y=test_mean, yerr=test_std, label='test', fmt='mo', ecolor='k' )
plt.errorbar( x=[ 2 ], y=holdout_mean, yerr=holdout_std, label='holdout', fmt='co', ecolor='k' )
